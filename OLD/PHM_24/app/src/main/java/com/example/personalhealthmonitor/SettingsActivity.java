...
public class SettingsActivity extends AppCompatActivity {
        ...
        toggleButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if( toggleButton.isChecked() ){
                    ...
                    Log.i(TAG, "First Run Settings Activity: " + firstRun );
    ...
    public void startTimedNotification(){
        ...
        //TODO INTERVAL_DAY -> una al giorno
    }
            ...
            if( updateVal > 2) {
                ...
                if (updateLowBound < updateUpBound) {  //inserisco nel DB
                    ...
                    if( Utils.checkDateSettings(updateBeginDate, updateEndDate) ){
                        ...
                        //TODO devo settare la notifica attiva per l'i-esimo parametro
                        /*
                        1 - In un nuovo thread
                        checkNelDB(dataBeg, dataENd, lowerB, upperB, parametro); -> {
                            double AVG = IVM -> @query nel db e mi ritorna il valore medio
                            if(AVG < low || AVG > upper){
                                    //si notifica
                                    EXCEEDED_TEMP (shared pref) = true -> settata come false all'inizio di tutto
                            }
                        }
                        2 - MONITORA_TEMP = true
                        3 - setOVER3 = true //c'è almeno un valore da monitorare (siamo dentro il > 2 )
...
//                MONITORA_TEMP (shared prefs) = settate a false
//                con uno switch??
                /*
                if( setOVER3 ){
                    //setto la notifica per le ore 9:00

                    //Intent che mi porta al Broadcast Receiver
                    Intent broadcastIntent = new Intent(this, AlertReceiver.class);
                    broadcastIntent.putExtra("notification_type", TIMED_NOTIFICATION);  SETTINGS_NOTIFICATION
                    PendingIntent pendingIntent = PendingIntent.getBroadcast(this, 1, broadcastIntent, PendingIntent.FLAG_UPDATE_CURRENT); //7 settimo intent credo
