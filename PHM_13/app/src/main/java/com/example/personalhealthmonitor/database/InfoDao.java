...
public interface InfoDao {
    ...
    //
//  @Query("SELECT * FROM report WHERE infoTemperatura != 0 ORDER BY infoTemperatura ASC")
    @Query("SELECT * FROM report WHERE infoTemperatura != 0 ORDER BY strftime('%d-%m-%Y', infoTemperatura) DESC")
    LiveData<List<Info>> getReportFromTemperature();


    @Query("SELECT * FROM report WHERE infoPressioneSistolica != 0 ORDER BY infoPressioneSistolica ASC")
    LiveData<List<Info>> getReportFromPressioneSistolica();

    @Query("SELECT * FROM report WHERE infoPressioneDiastolica != 0 ORDER BY infoPressioneDiastolica ASC")
    LiveData<List<Info>> getReportFromPressioneDiastolica();

    @Query("SELECT * FROM report WHERE infoGlicemia != 0 ORDER BY infoGlicemia ASC")
    LiveData<List<Info>> getReportFromGlicemia();

    @Query("SELECT * FROM report WHERE infoPeso != 0 ORDER BY infoPeso ASC")
